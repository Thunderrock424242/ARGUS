...
export const ingestionSubmissions = sqliteTable(
  "ingestion_submissions",
  {
    ...
    confidence: integer("confidence").notNull().default(25),
    confidenceUpdatedAt: text("confidence_updated_at"),
    confidenceUpdatedById: text("confidence_updated_by_id"),
    confidenceUpdatedByName: text("confidence_updated_by_name"),
    confidenceUpdateReason: text("confidence_update_reason"),
  ...
  (table) => [
    ...
    check("ingestion_submissions_confidence_check", sql`${table.confidence} between 0 and 99`),
...
export const collectorRuns = sqliteTable(
  "collector_runs",
  {
    ...
    errorMessage: text("error_message"),
    nextRetryAt: text("next_retry_at"),
    networkAccessed: integer("network_accessed", { mode: "boolean" }).notNull().default(false),
    requestId: text("request_id").notNull(),
    dataClassification: text("data_classification").notNull().default("demonstration"),
  },
  (table) => [
    ...
    index("collector_runs_retry_idx").on(table.status, table.nextRetryAt),
    check("collector_runs_attempt_check", sql`${table.attempt} >= 1`),
  ],
);
