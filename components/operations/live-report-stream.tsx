        ...
        {stream.slice(0, compact ? 10 : 40).map(({ report, event }) => {
          ...
          return (
            <article key={report.id} className={`grid gap-3 px-4 py-3 transition hover:bg-white/[.02] ${pinned.has(report.id) ? "bg-cyan-300/[.035]" : ""} sm:grid-cols-[78px_minmax(0,1fr)_auto]`}>
              ...
              <div className="flex items-start gap-2"><div className="text-right"><p className="font-mono text-xs text-slate-300">{event?.automatedConfidence ?? "—"}%</p><p className="text-[8px] uppercase tracking-[.12em] text-slate-600">confidence</p><p className="mt-2 text-[9px] text-slate-500">{event?.supportingSourceCount ?? 0} sources</p></div><button type="button" className={`icon-button ${pinned.has(report.id) ? "text-cyan-200" : ""}`} aria-label={`${pinned.has(report.id) ? "Unpin" : "Pin"} ${report.title}`} aria-pressed={pinned.has(report.id)} onClick={() => togglePin(report.id)}><Pin size={13} /></button></div>
            </article>
          );
        })}
      </div>
