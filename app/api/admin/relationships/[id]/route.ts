...
import { DurableOperationError, recordDurableRelationshipReview } from "@/packages/database/durable-operations";
...
export async function POST(request: Request, context: RelationshipRouteContext): Promise<Response> {
  ...
  if (!id.success) return jsonError(422, "invalid_relationship_id", "The relationship identifier is invalid.", { requestId, headers: guard.rateLimitHeaders });
