...
import { auditRecorder, createAuditEntry } from "@/lib/audit/recorder";
...
const collectorRunSchema = z
  .object({
    ...
    analystName: z.string().trim().min(1).max(100),
...
export async function POST(request: Request): Promise<Response> {
  ...
  const guard = await requireAdmin(request, "collector-run", requestId);
  ...
  try {
    ...
    const audit = createAuditEntry({
      ...
      actorName: body.data.analystName,
      summary: `${body.data.analystName} ran ${collector.name} in dry-run mode for ${source.name}.`,
      requestId,
      after: {
        runId: result.run.id,
    ...
    await auditRecorder.record(audit);
