...
export async function POST(request: Request, context: SeedRouteContext = {}): Promise<Response> {
  ...
  const guard = await requireAdmin(request, "demo-seed", requestId, context.adminToken);
  ...
  try {
    const result = await seedDurableDemonstrationData(context.database, body.data.reviewerName, requestId);
